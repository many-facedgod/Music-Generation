...
run_desc = "Nottingham_(pod)_13thDec"
run_id = None
log_file = None
...
music_file = "music_nottingham_train"
...
def validate_batch_perplexity(model, batch, make_batch_fn, criterion=CELoss()):
    ...
    with torch.no_grad():
        outputs = model(inp)
        loss = criterion(outputs, targets, reduce=False).sum().item()
        return loss, inp[1].sum().item()
...
def load_dataset(train_batch_size=16, eval_batch_size=32, train_ratio=1):
    """Loads the dataset
    ...
    data_val = np.load('../data/music_nottingham_test_data.npy')
...
def validate(model, val_dataset, criterion=CELoss(), make_batch_fn=make_batch, val_batch_fn=validate_batch_perplexity):
    ...
    for batch in bar:
        s, l = val_batch_fn(model, batch, make_batch_fn, criterion)
        score += s
        len_sum += l
        
        bar.set_description("Current validation score: {}".format(score/len_sum))
    log("Mean validation score: {}".format(score / len_sum))
    log("--------------------------------------------------------")
    return score / len_sum


def train(model, train_dataset, val_dataset, criterion, optimizer, iters, train_batch_fn=train_batch,
          val_batch_fn=validate_batch_perplexity, make_batch_fn=make_batch, save_every=5, validate_every=1,
          name="checkpoint"):
    """Trains the model
    ...
    for i in range(1, iters + 1):
        ...
        if not i % validate_every and val_batch_fn:
            ...
            validate(model, train_dataset, criterion, make_batch_fn, val_batch_fn)
