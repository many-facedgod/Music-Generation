...
class Baseline(nn.Module):
    ...
    def __init__(self, tuple_size, vocab_sizes, embedding_sizes=[256, 256, 256], n_layers=3, hidden_size=512, dropout=0.0):
        ...
        self.rnn = nn.LSTM(input_size=sum(embedding_sizes), num_layers=n_layers, hidden_size=hidden_size, dropout=dropout)
    ...
    def forward(self, x):
        ...
        logits = [self.scoring_layers[i](padded_outputs.view(-1, self.hidden_size)).view(max_len, batch_size, -1) for i in range(self.tuple_size)]
