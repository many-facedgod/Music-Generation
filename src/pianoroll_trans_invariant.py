...
class TransInvLoss(nn.Module):
    """Variable base notes make this one slightly tricky.
    """
...
def main():
    ...
    Runner.load_state("../experiments", 1544430672, ("checkpoint", 15), device, model)
    print(validate(model, testds, criterion))
    print(validate(model, trainds, criterion))
    for i in range(20):
        sample = model.sample()
        piano_matrix_to_midi(sample, "../outputs/transinv{}.mid".format(i))
